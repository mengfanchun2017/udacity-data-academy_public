...
ggplot(aes(x = CreditGrade), data = subset(pls2n, !is.na(CreditGrade))) + geom_histogram(stat = 'count') + xlab('CreditGrade') + ylab('Counts') + labs(title="信用等级统计") #可以和pls2对比，看下Null和NA的区别
#关于subset的使用还要再学习
...
##排除na数据
ggplot(aes(x = EmploymentStatusDuration), data = subset(pls2n, !is.na(EmploymentStatusDuration))) + geom_histogram(stat = 'count') + xlab('受雇用时间') + ylab('统计数量') + ggtitle('受雇用时间')
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
plot2a <- ggplot(aes(x = TotalCreditLinespast7years), data = subset(pls2o, !is.na(LoanStatus))) + geom_freqpoly(stat = 'count', aes(color = CreditGrade)) + ggtitle('7年内贷款次数与信用评级')
plot2b <- ggplot(aes(x = DelinquenciesLast7Years), data = subset(pls2o, !is.na(LoanStatus))) + geom_freqpoly(stat = 'count', aes(color = CreditGrade)) + xlim(1, 10) + ggtitle('7年内超期还款次数与信用评级')
grid.arrange(plot2a, plot2b, ncol = 1, top = '绘图2：贷款人信用评级')
```
